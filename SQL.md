...
优点：大大减小了服务器需要扫描的数据量，从而大大加快数据的检索速度

缺点：

- 创建索引和维护索引要耗费时间

- 索引需要占物理空间，聚簇索引尤其（索引存储在文件系统中

- 对表中的数据进行增、删、改的时候，也要动态地维护索引



建表时定义`PRIMARY KET`,`UNIQUE KEY`,`FOREIGN KEY`会自动生成对应的索引

使用复合索引（多列索引）时，注意最左前缀原则。查询条件中必须包含复合索引的最左列，才能使用索引
...
B+TREE：区别于B 树：所有数据存储在叶子节点，叶子节点通过指针连接，内部节点只存储键，不存储数据

HASH：采用一定的哈希算法，把键值换算成新的哈希值，检索时不需要类似B+树那样从根节点到叶子节点逐级查找，只需一次哈希算法即可立刻定位到相应的位置，速度非常快

- 只能满足` = `,` IN `和` <=> `查询，不能使用范围查询

FULLTEXT：全文索引，用于文本内容的全文搜索

SPATIAL：空间索引，用于地理空间数据



聚簇索引：-- InnoDB 表总是有聚簇索引

- 一个表只能有一个聚簇索引
- 表数据本身就是索引的一部分，叶子节点包含完整的数据行
- 数据按索引键值顺序存储
- 直接定位到数据，无需二次查找
...
聚簇索引 B+树结构：
        ...
        中间节点
          |
叶子节点 → [id=1, name="John", dept="IT", salary=50000]
叶子节点 → [id=2, name="Jane", dept="HR", salary=45000]
叶子节点 → [id=3, name="Bob", dept="IT", salary=60000]
```

非聚簇索引：-- MyISAM 只有非聚簇索引

- 索引和数据分离，索引中只包含指向数据的指针，叶子节点不包含完整数据
- 一个表可以有多个
- 需要回表查询：先找索引，再找数据
- 逻辑有序：索引本身有序，但数据无序
...
非聚簇索引 B+树结构：
        ...
        中间节点
          |
叶子节点 → [dept="HR", primary_key=2]
叶子节点 → [dept="IT", primary_key=1]
叶子节点 → [dept="IT", primary_key=3]

数据物理存储（按主键顺序）：
[id=1, name="John", dept="IT", salary=50000]
[id=2, name="Jane", dept="HR", salary=45000]  
[id=3, name="Bob", dept="IT", salary=60000]
```
